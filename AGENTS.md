...
- Internal `client-docs` `🚦 Ready to Release` means local QA passed but David has not yet approved and Release Manager has not yet published the candidate. Keep it publicly in `next`; never present it as `in_review` or `done`.
