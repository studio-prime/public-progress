...
- Client-safe attachments can be included with an `attachments` array. Use `type: "pdf"`, `"image"`, or `"link"` plus a short `label` and `url`. Store public files under `assets/<client-slug>/...` and reference that relative path. Use external `https://` links only when the linked resource is safe for clients.
